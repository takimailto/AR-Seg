...
class VectorQuantizer2(nn.Module):
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums): # from small to large
                ...
                if self.using_znorm:
                    ...
                    d_no_grad = torch.addmm(d_no_grad, rest_NC, self.embedding.weight.data.T, alpha=-2, beta=1)  # (B*h*w, vocab_size)
                ...
                if self.training and dist.initialized():
                    handler.wait()
                    if self.record_hit == 0: self.ema_vocab_hit_SV[si] = hit_V
                    elif self.record_hit < 100: self.ema_vocab_hit_SV[si] = self.ema_vocab_hit_SV[si]*0.9 + hit_V.mul(0.1)
                    else: self.ema_vocab_hit_SV[si] = self.ema_vocab_hit_SV[si] * 0.99 + hit_V.mul(0.01)
                    self.record_hit += 1
                vocab_hit_V = vocab_hit_V + hit_V
                mean_vq_loss += F.mse_loss(f_hat.data, f_BChw) * self.beta + F.mse_loss(f_hat, f_no_grad)
        ...
        return f_hat, usages, mean_vq_loss, f_hat_list
    # ===================== `forward` is only used in VAE training =====================
    ...
    def f_to_idxBl_or_fhat(self, f_BChw: torch.Tensor, to_fhat: bool, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> List[Union[torch.Tensor, torch.LongTensor]]:  # z_BChw is the feature from inp_img_no_grad
        ...
        for si, (ph, pw) in enumerate(patch_hws): # from small to large
            ...
            if self.using_znorm:
                ...
                d_no_grad = torch.addmm(d_no_grad, z_NC, self.embedding.weight.data.T, beta=1, alpha=-2)
                idx_N = torch.argmin(d_no_grad, dim=1)
    ...
    def idxBl_to_var_input(self, gt_ms_idx_Bl: List[torch.Tensor]) -> torch.Tensor:
        ...
        for si in range(SN-1):
            ...
            h_BChw = F.interpolate(self.embedding(gt_ms_idx_Bl[si]).transpose(1, 2).view(B, C, pn_next, pn_next), size=(H, W), mode='bicubic')
